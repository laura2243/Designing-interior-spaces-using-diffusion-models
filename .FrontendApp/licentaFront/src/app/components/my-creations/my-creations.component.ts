...
export class MyCreationsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.inpaintBackService.getMyCollectionImages(
    ).pipe().subscribe(data => {
      this.images = data

      this.allImagesBeforeConversion = data
      this.allImages = data.map((image: any) => this.convertAtributes(image)).reverse()
  ...
  clickOnImage(image: ImageInpaintModelResult) {
    ...
    this.router.navigate(['/image-details'], { state: { data: image, fromGallery: false } })
  }
